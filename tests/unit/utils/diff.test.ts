...
import { buildLineDiffLines, diffFromToolInput, extractDiffData, parseApplyPatchDiffs } from '../../../src/utils/diff';
...
describe('buildLineDiffLines', () => {
  it('builds stable equal, delete, and insert lines with line numbers', () => {
    const result = buildLineDiffLines('a\nb\nc', 'a\nc\nd');

    expect(result).toEqual([
      { type: 'equal', text: 'a', oldLineNum: 1, newLineNum: 1 },
      { type: 'delete', text: 'b', oldLineNum: 2 },
      { type: 'equal', text: 'c', oldLineNum: 3, newLineNum: 2 },
      { type: 'insert', text: 'd', newLineNum: 3 },
    ]);
  });

  it('handles empty trailing lines consistently', () => {
    const result = buildLineDiffLines('a\n', 'a\nb');

    expect(result).toEqual([
      { type: 'equal', text: 'a', oldLineNum: 1, newLineNum: 1 },
      { type: 'delete', text: '', oldLineNum: 2 },
