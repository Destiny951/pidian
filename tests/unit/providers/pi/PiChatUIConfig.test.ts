...
describe('PiChatUIConfig', () => {
  describe('getModelOptions', () => {
    it('should return available models', async () => {
      const options = await piChatUIConfig.getModelOptions({});
  ...
  describe('getPermissionModeToggle', () => {
    it('should return toggle config with normal/yolo values', () => {
      const config = piChatUIConfig.getPermissionModeToggle?.();
      expect(config).not.toBeNull();
      expect(config!.inactiveValue).toBe('normal');
      expect(config!.activeValue).toBe('yolo');
    });
  });
