...
function createMockUIConfig() {
  return {
    getModelOptions: jest.fn().mockImplementation(async (settings: {
      enableOpus1M?: boolean;
      enableSonnet1M?: boolean;
...
describe('ModelSelector', () => {
  ...
  it('should create a container with model-selector class', () => {
    const container = parentEl.querySelector('.pidian-model-selector');
    expect(container).not.toBeNull();
  });

  it('should display current model label', () => {
    // Default model is 'sonnet' which maps to 'Sonnet'
    const btn = parentEl.querySelector('.pidian-model-btn');
    expect(btn).not.toBeNull();
    const label = btn?.querySelector('.pidian-model-label');
    expect(label).not.toBeNull();
    expect(label?.textContent).toBe('Sonnet');
  });

  it('should display first model when current model not found', async () => {
    ...
    await selector.updateDisplay();
    const label = parentEl.querySelector('.pidian-model-label');
    expect(label?.textContent).toBe('Haiku');
  });

  it('should render model options in reverse order', async () => {
    const dropdown = parentEl.querySelector('.pidian-model-dropdown');
  ...
  it('should mark current model as selected', async () => {
    const dropdown = parentEl.querySelector('.pidian-model-dropdown');
    const options = dropdown?.children || [];
  ...
  it('should use custom models from environment variables', async () => {
  ...
  it('should not filter custom env models when 1M toggles are enabled', async () => {
    callbacks.getEnvironmentVariables.mockReturnValue(
  ...
  it('should render group separators when models have group field', async () => {
    const groupedModels = [
    ...
    uiConfig.getModelOptions.mockResolvedValue(groupedModels);
    ...
    const groups = children.filter((c: any) => c.hasClass('pidian-model-group'));
    expect(groups.length).toBe(2);
    expect(groups[0]?.textContent).toBe('Codex');
    expect(groups[1]?.textContent).toBe('Claude');
  });

  it('should not render group separators when models have no group field', async () => {
    await selector.renderOptions();
  ...
  it('should show 1M variants instead of standard variants when enabled', async () => {
    callbacks.getSettings.mockReturnValue({
      model: 'opus[1m]',
    ...
    expect(parentEl.querySelector('.pidian-model-label')?.textContent).toBe('Opus 1M');
  });
});
...
describe('ThinkingBudgetSelector', () => {
  ...
  describe('adaptive mode (Claude models)', () => {
    ...
    it('should create a container with thinking-selector class', () => {
      const container = parentEl.querySelector('.pidian-thinking-selector');
      expect(container).not.toBeNull();
    });

    it('should show effort selector for Claude models', () => {
      const effort = parentEl.querySelector('.pidian-thinking-effort');
      expect(effort).not.toBeNull();
      expect(effort?.style?.display).not.toBe('none');
    });

    it('should hide budget selector for Claude models', () => {
      const budget = parentEl.querySelector('.pidian-thinking-budget');
      expect(budget?.style?.display).toBe('none');
    });

    it('should display current effort level for Claude models', () => {
      const current = parentEl.querySelector('.pidian-thinking-current');
  ...
  describe('legacy mode (custom models)', () => {
    ...
    it('should display Off when budget is off', async () => {
      callbacks.getSettings.mockReturnValue({
        model: 'custom-model',
        thinkingBudget: 'off',
        serviceTier: 'default',
    ...
    it('should render budget options in reverse order', () => {
      const options = parentEl.querySelector('.pidian-thinking-options');
...
describe('PermissionToggle', () => {
  ...
  it('should create a container with permission-toggle class', () => {
    const container = parentEl.querySelector('.pidian-permission-toggle');
    expect(container).not.toBeNull();
  });

  it('should display Safe label when in normal mode', () => {
    const label = parentEl.querySelector('.pidian-permission-label');
  ...
  it('should display YOLO label when in yolo mode', () => {
    ...
    const label = parentEl2.querySelector('.pidian-permission-label');
  ...
  it('should show PLAN label and hide toggle in plan mode', () => {
    ...
    const toggle = parentEl2.querySelector('.pidian-toggle-switch');
  ...
  it('should not have active class in normal mode', () => {
    const toggle = parentEl.querySelector('.pidian-toggle-switch');
  ...
  it('should hide the control when provider exposes no permission toggle UI', () => {
    ...
    const container = parentEl2.querySelector('.pidian-permission-toggle');
...
describe('ServiceTierToggle', () => {
  ...
  it('shows the control when the provider exposes service tier options', () => {
    const container = parentEl.querySelector('.pidian-service-tier-toggle');
    expect(container).not.toBeNull();
    expect(container?.style.display).toBe('');
  });

  it('renders the icon button in the inactive state when fast mode is off', () => {
    const button = parentEl.querySelector('.pidian-service-tier-button');
    const icon = parentEl.querySelector('.pidian-service-tier-icon');
    const container = parentEl.querySelector('.pidian-service-tier-toggle');
  ...
  it('renders the icon button in the active state when fast mode is on', () => {
    ...
    const button = parentEl2.querySelector('.pidian-service-tier-button');
    const container = parentEl2.querySelector('.pidian-service-tier-toggle');
...
describe('McpServerSelector', () => {
  ...
  it('should create container with mcp-selector class', () => {
    const container = parentEl.querySelector('.pidian-mcp-selector');
    expect(container).not.toBeNull();
  });
  ...
  it('should show empty message when all servers are disabled', () => {
    ...
    const empty = parentEl.querySelector('.pidian-mcp-selector-empty');
  ...
  it('should show badge when more than 1 server enabled', () => {
    ...
    const badge = parentEl.querySelector('.pidian-mcp-selector-badge');
  ...
  it('should add active class to icon when servers are enabled', () => {
    ...
    const icon = parentEl.querySelector('.pidian-mcp-selector-icon');
...
describe('ContextUsageMeter', () => {
  ...
  it('should create a container with context-meter class', () => {
    const container = parentEl.querySelector('.pidian-context-meter');
    expect(container).not.toBeNull();
  });
  ...
  it('should render only the circle gauge without percentage text', () => {
    meter.update(makeUsage({ contextTokens: 50000, contextWindow: 200000, percentage: 25 }));
    const percent = parentEl.querySelector('.pidian-context-meter-percent');
    const gauge = parentEl.querySelector('.pidian-context-meter-gauge');
...
describe('McpServerSelector - toggle and badges', () => {
  ...
  it('should render context-saving badge for servers with contextSaving', () => {
    ...
    const csBadge = parentEl.querySelector('.pidian-mcp-selector-cs-badge');
  ...
  it('should toggle server on mousedown and update display', () => {
    ...
    const item = parentEl.querySelector('.pidian-mcp-selector-item');
