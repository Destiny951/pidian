...
describe('builtInCommands', () => {
  ...
  describe('getBuiltInCommandsForDropdown - provider filtering', () => {
    ...
    it('returns only commands supported by PI capabilities', () => {
      const commands = getBuiltInCommandsForDropdown('pi');
  ...
  describe('isBuiltInCommandSupported', () => {
    it('returns true for universal commands on any provider', () => {
      const clearCmd = BUILT_IN_COMMANDS.find((c) => c.name === 'clear')!;
      expect(isBuiltInCommandSupported(clearCmd, 'pi')).toBe(true);
    });

    it('returns false for capability-restricted commands when PI does not support them', () => {
      const resumeCmd = BUILT_IN_COMMANDS.find((c) => c.name === 'resume')!;
      expect(isBuiltInCommandSupported(resumeCmd, 'pi')).toBe(false);
