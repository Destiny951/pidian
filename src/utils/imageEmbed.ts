...
function createImageHtml(
  ...
  return `<span class="pidian-embedded-image"><img src="${escapeHtml(src)}" alt="${alt}" loading="lazy"${style}></span>`;
}

function createFallbackHtml(wikilink: string): string {
  return `<span class="pidian-embedded-image-fallback">${escapeHtml(wikilink)}</span>`;
}
