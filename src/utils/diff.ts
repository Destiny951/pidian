...
export function buildLineDiffLines(original: string, proposed: string): DiffLine[] {
  ...
  while (i > 0 || j > 0) {
    ...
    if (direction === DIFF_DIAGONAL) {
      diffLines.push({ type: 'equal', text: originalLines[i - 1], oldLineNum: i, newLineNum: j });
      i--;
      j--;
    } else if (j > 0 && (i === 0 || direction === DIFF_LEFT)) {
      diffLines.push({ type: 'insert', text: proposedLines[j - 1], newLineNum: j });
      j--;
    } else {
      diffLines.push({ type: 'delete', text: originalLines[i - 1], oldLineNum: i });
      i--;
    }
  }
...
export function reconstructFromDiff(diffLines: DiffLine[]): { original: string; proposed: string } {
