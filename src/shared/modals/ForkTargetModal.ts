...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('pidian-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'pidian-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'pidian-fork-target-option', text: label });
