...
export class MentionDropdownController {
  ...
  constructor(
    ...
    this.dropdown = new SelectableDropdown<MentionItem>(this.containerEl, {
      listClassName: 'pidian-mention-dropdown',
      itemClassName: 'pidian-mention-item',
      emptyClassName: 'pidian-mention-empty',
      fixed: this.fixed,
      fixedClassName: 'pidian-mention-dropdown-fixed',
    });
  }
  ...
  private renderMentionDropdown(): void {
    this.dropdown.render({
      ...
      renderItem: (item, itemEl) => {
        const iconEl = itemEl.createSpan({ cls: 'pidian-mention-icon' });
        switch (item.type) {
          case 'mcp-server':
        ...
        const textEl = itemEl.createSpan({ cls: 'pidian-mention-text' });

        switch (item.type) {
          case 'mcp-server':
            textEl.createSpan({ cls: 'pidian-mention-name' }).setText(`@${item.name}`);
            break;
          case 'agent-folder':
            textEl.createSpan({
              cls: 'pidian-mention-name pidian-mention-name-agent-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'agent': {
            // Show ID (which is namespaced for plugin agents) for consistency with inserted text
            textEl.createSpan({
              cls: 'pidian-mention-name pidian-mention-name-agent',
            }).setText(`@${item.id}`);
            if (item.description) {
              textEl.createSpan({ cls: 'pidian-mention-agent-desc' }).setText(item.description);
            }
            break;
          }
          case 'context-folder':
            textEl.createSpan({
              cls: 'pidian-mention-name pidian-mention-name-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'context-file':
            textEl.createSpan({
              cls: 'pidian-mention-name pidian-mention-name-context',
            }).setText(item.name);
            break;
          case 'folder':
            textEl.createSpan({
            ...
            break;
          default:
            textEl.createSpan({ cls: 'pidian-mention-path' }).setText(item.path || item.name);
        }
      },
