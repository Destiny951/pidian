...
export class SlashCommandDropdown {
  ...
  private render(): void {
    ...
    if (this.filteredItems.length === 0) {
      const emptyEl = this.dropdownEl.createDiv({ cls: 'pidian-slash-empty' });
      emptyEl.setText('No matching commands');
    } else {
      for (let i = 0; i < this.filteredItems.length; i++) {
        const item = this.filteredItems[i];
        const itemEl = this.dropdownEl.createDiv({ cls: 'pidian-slash-item' });

        if (i === this.selectedIndex) {
          itemEl.addClass('selected');
        }

        const nameEl = itemEl.createSpan({ cls: 'pidian-slash-name' });
        nameEl.setText(`${item.displayPrefix}${item.name}`);

        if (item.argumentHint) {
          const hintEl = itemEl.createSpan({ cls: 'pidian-slash-hint' });
          hintEl.setText(normalizeArgumentHint(item.argumentHint));
        }

        if (item.description) {
          const descEl = itemEl.createDiv({ cls: 'pidian-slash-desc' });
  ...
  private createDropdownElement(): HTMLElement {
    if (this.isFixed) {
      return this.containerEl.createDiv({
        cls: 'pidian-slash-dropdown pidian-slash-dropdown-fixed',
      });
    } else {
      return this.containerEl.createDiv({ cls: 'pidian-slash-dropdown' });
    }
  }
  ...
  private updateSelection(): void {
    const items = this.dropdownEl?.querySelectorAll('.pidian-slash-item');
