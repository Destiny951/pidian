...
import { DEFAULT_PIDIAN_SETTINGS } from './app/settings/defaultSettings';
...
import {
  VIEW_TYPE_DIFF_APPROVAL,
  VIEW_TYPE_PIDIAN,
} from './core/types';
import type { EnvironmentScope } from './core/types/settings';
import { PidianView } from './features/chat/PidianView';
import { DiffApprovalView } from './features/diff-view/DiffApprovalView';
import { type InlineEditContext, InlineEditModal } from './features/inline-edit/ui/InlineEditModal';
import { PidianSettingTab } from './features/settings/PidianSettings';
...
export default class PidianPlugin extends Plugin {
  settings!: PidianSettings;
  storage!: SharedAppStorage;
  private conversations: Conversation[] = [];

  async onload() {
    // Initialize PI plugin instance for model fetching
    const { setPluginInstance } = await import('./providers/pi/ui/PiChatUIConfig');
    ...
    this.registerView(
      VIEW_TYPE_PIDIAN,
      (leaf) => new PidianView(leaf, this)
    );

    this.registerView(
      VIEW_TYPE_DIFF_APPROVAL,
      (leaf) => new DiffApprovalView(leaf, this)
    );

    this.addRibbonIcon('bot', 'Open Pidian', () => {
      this.activateView();
    });
    ...
    this.addCommand({
      ...
      checkCallback: (checking: boolean) => {
        const leaf = this.app.workspace.getLeavesOfType(VIEW_TYPE_PIDIAN)[0];
        if (!leaf) return false;

        const view = leaf.view as PidianView;
  ...
  async activateView() {
    ...
    let leaf = workspace.getLeavesOfType(VIEW_TYPE_PIDIAN)[0];
  ...
  async loadSettings() {
    ...
    const didNormalizeModelVariants = await this.normalizeModelVariantSettings();
    ...
    const { changed, invalidatedConversations } = await this.reconcileModelWithEnvironment();

    await ProviderSettingsCoordinator.projectActiveProviderState(
      this.settings as unknown as Record<string, unknown>,
    );
  ...
  async normalizeModelVariantSettings(): Promise<boolean> {
    return await ProviderSettingsCoordinator.normalizeAllModelVariants(
      this.settings as unknown as Record<string, unknown>,
    );
  }
  ...
  async saveSettings() {
    ...
    await this.storage.savePidianSettings(this.settings);
  }
  ...
  async applyEnvironmentVariablesBatch(
    ...
    const { changed, invalidatedConversations } = await this.reconcileModelWithEnvironment(affectedProviderIds);
    await this.saveSettings();
  ...
  private async reconcileModelWithEnvironment(providerIds: ProviderId[] = ProviderRegistry.getRegisteredProviderIds()): Promise<{
    changed: boolean;
    invalidatedConversations: Conversation[];
  }> {
    return await ProviderSettingsCoordinator.reconcileProviders(
      this.settings as unknown as Record<string, unknown>,
      this.conversations,
      providerIds,
    );
  ...
  getView(): PidianView | null {
    const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE_PIDIAN);
    if (leaves.length > 0) {
      return leaves[0].view as PidianView;
    }
    return null;
  }

  getAllViews(): PidianView[] {
    const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE_PIDIAN);
    return leaves.map(leaf => leaf.view as PidianView);
  }

  findConversationAcrossViews(conversationId: string): { view: PidianView; tabId: string } | null {
    for (const view of this.getAllViews()) {
      const tabManager = view.getTabManager();
      if (!tabManager) continue;
