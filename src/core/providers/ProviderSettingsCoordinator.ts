...
export class ProviderSettingsCoordinator {
  ...
  static async getProviderSettingsSnapshot<T extends Record<string, unknown>>(
    settings: T,
    providerId: ProviderId,
  ): Promise<T> {
    const snapshot = cloneProviderSettings(settings) as T;
    await this.projectProviderState(snapshot, providerId);
    return snapshot;
  }
  ...
  static async projectProviderState(
    settings: Record<string, unknown>,
    providerId: ProviderId,
  ): Promise<void> {
  ...
  static async reconcileProviders(
    settings: Record<string, unknown>,
    conversations: Conversation[],
    ...
    for (const providerId of providerIds) {
      ...
      if (providerId !== settingsProvider) {
        await this.projectProviderState(targetSettings, providerId);
      }
  ...
  static async normalizeAllModelVariants(settings: Record<string, unknown>): Promise<boolean> {
    let anyChanged = false;
    const settingsProvider = getSettingsProviderId(settings);
   ...
   */
  static async projectActiveProviderState(settings: Record<string, unknown>): Promise<void> {
    await this.projectProviderState(settings, getSettingsProviderId(settings));
  }
}
