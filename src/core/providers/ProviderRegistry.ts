...
export class ProviderRegistry {
  ...
  static createTitleGenerationService(plugin: PidianPlugin, providerId: ProviderId = DEFAULT_CHAT_PROVIDER_ID): TitleGenerationService {
    return this.getProviderRegistration(providerId).createTitleGenerationService(plugin);
  }

  static createInstructionRefineService(plugin: PidianPlugin, providerId: ProviderId = DEFAULT_CHAT_PROVIDER_ID): InstructionRefineService {
    return this.getProviderRegistration(providerId).createInstructionRefineService(plugin);
  }

  static createInlineEditService(plugin: PidianPlugin, providerId: ProviderId = DEFAULT_CHAT_PROVIDER_ID): InlineEditService {
  ...
  static getEnabledProviderIds(settings: Record<string, unknown>): ProviderId[] {
    return this.isEnabled(DEFAULT_CHAT_PROVIDER_ID, settings) ? [DEFAULT_CHAT_PROVIDER_ID] : [];
  }
  ...
  static getCustomModelIds(envVars: Record<string, string>): Set<string> {
    return this.getChatUIConfig(DEFAULT_CHAT_PROVIDER_ID).getCustomModelIds(envVars);
  }
}
