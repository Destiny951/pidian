...
function addHotkeySettingRow(
  ...
  const item = containerEl.createDiv({ cls: 'pidian-hotkey-item' });
  item.createSpan({
    cls: 'pidian-hotkey-name',
    text: t(`${translationPrefix}.name` as TranslationKey),
  });
  if (hotkey) {
    item.createSpan({ cls: 'pidian-hotkey-badge', text: hotkey });
  }
...
export class PidianSettingTab extends PluginSettingTab {
  ...
  constructor(app: App, plugin: PidianPlugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  ...
  display(): void {
    ...
    containerEl.empty();
    containerEl.addClass('pidian-settings');

    setLocale(this.plugin.settings.locale as Locale);

    const providerTabs = ProviderRegistry.getRegisteredProviderIds().filter(
      (providerId) => ProviderWorkspaceRegistry.getSettingsTabRenderer(providerId),
    );
    ...
    const tabBar = containerEl.createDiv({ cls: 'pidian-settings-tabs' });
    ...
    for (const id of tabIds) {
      ...
      const button = tabBar.createEl('button', {
        cls: `pidian-settings-tab${id === this.activeTab ? ' pidian-settings-tab--active' : ''}`,
        text: label,
      });
      button.addEventListener('click', () => {
        this.activeTab = id;
        for (const tabId of tabIds) {
          tabButtons.get(tabId)?.toggleClass('pidian-settings-tab--active', tabId === id);
          tabContents.get(tabId)?.toggleClass('pidian-settings-tab-content--active', tabId === id);
        }
      });
    ...
    for (const id of tabIds) {
      const content = containerEl.createDiv({
        cls: `pidian-settings-tab-content${id === this.activeTab ? ' pidian-settings-tab-content--active' : ''}`,
      });
  ...
  private renderGeneralTab(container: HTMLElement): void {
    ...
    const maxTabsWarningEl = container.createDiv({ cls: 'pidian-max-tabs-warning' });
      ...
      new Setting(container)
        ...
        .addDropdown(async (dropdown) => {
          ...
          for (const providerId of ProviderRegistry.getRegisteredProviderIds()) {
            ...
            const models = await uiConfig.getModelOptions(settingsBag);
    ...
    new Setting(container)
      ...
      .addText((text) => {
        ...
        text.inputEl.addClass('pidian-settings-media-input');
        text.inputEl.addEventListener('blur', () => this.restartServiceForPromptChange());
      });
    ...
    const hotkeyGrid = container.createDiv({ cls: 'pidian-hotkey-grid' });
    addHotkeySettingRow(hotkeyGrid, this.app, 'pidian:inline-edit', 'settings.inlineEditHotkey');
    addHotkeySettingRow(hotkeyGrid, this.app, 'pidian:open-view', 'settings.openChatHotkey');
    addHotkeySettingRow(hotkeyGrid, this.app, 'pidian:new-session', 'settings.newSessionHotkey');
    addHotkeySettingRow(hotkeyGrid, this.app, 'pidian:new-tab', 'settings.newTabHotkey');
    addHotkeySettingRow(hotkeyGrid, this.app, 'pidian:close-current-tab', 'settings.closeTabHotkey');
  ...
  private renderCustomContextLimits(container: HTMLElement, providerId?: ProviderId): void {
    ...
    const headerEl = container.createDiv({ cls: 'pidian-context-limits-header' });
    headerEl.createSpan({
      text: t('settings.customContextLimits.name'),
      cls: 'pidian-context-limits-label',
    });

    const descEl = container.createDiv({ cls: 'pidian-context-limits-desc' });
    descEl.setText(t('settings.customContextLimits.desc'));

    const listEl = container.createDiv({ cls: 'pidian-context-limits-list' });

    for (const modelId of uniqueModelIds) {
      const currentValue = this.plugin.settings.customContextLimits?.[modelId];

      const itemEl = listEl.createDiv({ cls: 'pidian-context-limits-item' });
      const nameEl = itemEl.createDiv({ cls: 'pidian-context-limits-model' });
      nameEl.setText(modelId);

      const inputWrapper = itemEl.createDiv({ cls: 'pidian-context-limits-input-wrapper' });
      const inputEl = inputWrapper.createEl('input', {
        type: 'text',
        placeholder: '200k',
        cls: 'pidian-context-limits-input',
        value: currentValue ? formatContextLimit(currentValue) : '',
      });

      const validationEl = inputWrapper.createDiv({ cls: 'pidian-context-limit-validation' });
      ...
      inputEl.addEventListener('input', async () => {
        ...
        if (!trimmed) {
          ...
          inputEl.classList.remove('pidian-input-error');
        } else {
          const parsed = parseContextLimit(trimmed);
          if (parsed === null) {
            validationEl.setText(t('settings.customContextLimits.invalid'));
            validationEl.style.display = 'block';
            inputEl.classList.add('pidian-input-error');
            return;
          }
