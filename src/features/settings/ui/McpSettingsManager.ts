...
export class McpSettingsManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'pidian-mcp-header' });
    headerEl.createSpan({ text: 'MCP Servers', cls: 'pidian-mcp-label' });

    const addContainer = headerEl.createDiv({ cls: 'pidian-mcp-add-container' });
    const addBtn = addContainer.createEl('button', {
      cls: 'pidian-settings-action-btn',
      attr: { 'aria-label': 'Add' },
    });
    setIcon(addBtn, 'plus');

    const dropdown = addContainer.createDiv({ cls: 'pidian-mcp-add-dropdown' });

    const stdioOption = dropdown.createDiv({ cls: 'pidian-mcp-add-option' });
    setIcon(stdioOption.createSpan({ cls: 'pidian-mcp-add-option-icon' }), 'terminal');
    ...
    const httpOption = dropdown.createDiv({ cls: 'pidian-mcp-add-option' });
    setIcon(httpOption.createSpan({ cls: 'pidian-mcp-add-option-icon' }), 'globe');
    ...
    const importOption = dropdown.createDiv({ cls: 'pidian-mcp-add-option' });
    setIcon(importOption.createSpan({ cls: 'pidian-mcp-add-option-icon' }), 'clipboard-paste');
    ...
    if (this.servers.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'pidian-mcp-empty' });
      emptyEl.setText('No MCP servers configured. Click "Add" to add one.');
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'pidian-mcp-list' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'pidian-mcp-item' });
    if (!server.enabled) {
      itemEl.addClass('pidian-mcp-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'pidian-mcp-status' });
    statusEl.addClass(
      server.enabled ? 'pidian-mcp-status-enabled' : 'pidian-mcp-status-disabled'
    );

    const infoEl = itemEl.createDiv({ cls: 'pidian-mcp-info' });

    const nameRow = infoEl.createDiv({ cls: 'pidian-mcp-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'pidian-mcp-name' });
    nameEl.setText(server.name);

    const serverType = getMcpServerType(server.config);
    const typeEl = nameRow.createSpan({ cls: 'pidian-mcp-type-badge' });
    typeEl.setText(serverType);

    if (server.contextSaving) {
      const csEl = nameRow.createSpan({ cls: 'pidian-mcp-context-saving-badge' });
      csEl.setText('@');
      csEl.setAttribute('title', 'Context-saving: mention with @' + server.name + ' to enable');
    }

    const previewEl = infoEl.createDiv({ cls: 'pidian-mcp-preview' });
    ...
    const actionsEl = itemEl.createDiv({ cls: 'pidian-mcp-actions' });

    const testBtn = actionsEl.createEl('button', {
      cls: 'pidian-mcp-action-btn',
    ...
    const deleteBtn = actionsEl.createEl('button', {
      cls: 'pidian-mcp-action-btn pidian-mcp-delete-btn',
