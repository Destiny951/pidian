...
export function renderEnvironmentSettingsSection(
  ...
  const reviewEl = container.createDiv({ cls: 'pidian-env-review-warning' });
  ...
  new Setting(container)
    ...
    .addTextArea((text) => {
      ...
      text.inputEl.addClass('pidian-settings-env-textarea');
  ...
  const contextLimitsContainer = container.createDiv({ cls: 'pidian-context-limits-container' });
  renderCustomContextLimits?.(contextLimitsContainer);

  const envSnippetsContainer = container.createDiv({ cls: 'pidian-env-snippets-container' });
