...
import type { PidianView } from '../../chat/PidianView';
...
export class EnvSnippetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('pidian-env-snippet-modal');
    ...
    const renderContextLimitFields = () => {
      ...
      for (const modelId of uniqueModelIds) {
        const row = contextLimitsContainer.createDiv({ cls: 'pidian-snippet-limit-row' });
        row.createSpan({ text: modelId, cls: 'pidian-snippet-limit-model' });
        row.createSpan({ cls: 'pidian-snippet-limit-spacer' });

        const input = row.createEl('input', {
          type: 'text',
          placeholder: '200k',
          cls: 'pidian-snippet-limit-input',
        });
    ...
    envVarsSetting.settingEl.addClass('pidian-env-snippet-setting');
    envVarsSetting.controlEl.addClass('pidian-env-snippet-control');

    contextLimitsContainer = contentEl.createDiv({ cls: 'pidian-snippet-context-limits' });
    renderContextLimitFields();

    const buttonContainer = contentEl.createDiv({ cls: 'pidian-snippet-buttons' });

    const cancelBtn = buttonContainer.createEl('button', {
      text: t('settings.envSnippets.modal.cancel'),
      cls: 'pidian-cancel-btn'
    });
...
export class EnvSnippetManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'pidian-snippet-header' });
    headerEl.createSpan({ text: t('settings.envSnippets.name'), cls: 'pidian-snippet-label' });
    ...
    if (snippets.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'pidian-snippet-empty' });
      emptyEl.setText(t('settings.envSnippets.noSnippets'));
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'pidian-snippet-list' });

    for (const snippet of snippets) {
      const itemEl = listEl.createDiv({ cls: 'pidian-snippet-item' });

      const infoEl = itemEl.createDiv({ cls: 'pidian-snippet-info' });

      const nameEl = infoEl.createDiv({ cls: 'pidian-snippet-name' });
      nameEl.setText(snippet.name);

      if (snippet.description) {
        const descEl = infoEl.createDiv({ cls: 'pidian-snippet-description' });
        descEl.setText(snippet.description);
      }

      const actionsEl = itemEl.createDiv({ cls: 'pidian-snippet-actions' });
      ...
      const deleteBtn = actionsEl.createEl('button', {
        cls: 'pidian-settings-action-btn pidian-settings-delete-btn',
  ...
  private async insertSnippet(snippet: EnvSnippet) {
    ...
    const view = this.plugin.app.workspace.getLeavesOfType('pidian-view')[0]?.view as PidianView | undefined;
  ...
  private syncTextareaValue(scope: EnvironmentScope, value: string): void {
    const selector = `.pidian-settings-env-textarea[data-env-scope="${scope}"]`;
