...
export class TabBar {
  ...
  private build(): void {
    this.containerEl.addClass('pidian-tab-badges');
  }
  ...
  private renderBadge(item: TabBarItem): void {
    ...
    let stateClass = 'pidian-tab-badge-idle';
    if (item.isActive) {
      stateClass = 'pidian-tab-badge-active';
    } else if (item.needsAttention) {
      stateClass = 'pidian-tab-badge-attention';
    } else if (item.isStreaming) {
      stateClass = 'pidian-tab-badge-streaming';
    }

    const badgeEl = this.containerEl.createDiv({
      cls: `pidian-tab-badge ${stateClass}`,
  ...
  destroy(): void {
    this.containerEl.empty();
    this.containerEl.removeClass('pidian-tab-badges');
  }
}
