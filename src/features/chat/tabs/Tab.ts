...
import type { ToolbarSettings } from '../ui/InputToolbar';
...
function parsePiModelValue(model: string): { provider: string; modelId: string } {
  const separator = model.indexOf('/');
  if (separator < 0) {
    return { provider: 'pi', modelId: model };
  }
...
async function switchPiDefaultModel(plugin: PidianPlugin, tab: TabData, model: string): Promise<boolean> {
  const { provider, modelId } = parsePiModelValue(model);
  if (!provider || !modelId) {
    new Notice('Invalid PI model selection');
    return false;
  }

  if (tab.service?.providerId === 'pi' && tab.service.setModel) {
 ...
 */
export async function getBlankTabModelOptions(
  settings: Record<string, unknown>,
): Promise<ProviderUIOption[]> {
  const uiConfig = ProviderRegistry.getChatUIConfig(DEFAULT_CHAT_PROVIDER_ID);
  const providerIcon = uiConfig.getProviderIcon?.() ?? undefined;
  const models = await uiConfig.getModelOptions(settings);
  return models.map(model => ({
    ...model,
    providerIcon: model.providerIcon ?? providerIcon,
  }));
}
 ...
 */
async function resolveBlankTabModel(
  plugin: PidianPlugin,
  providerId?: ProviderId,
): Promise<string> {
  const settings = plugin.settings as unknown as Record<string, unknown>;
  if (providerId) {
    const snapshot = await ProviderSettingsCoordinator.getProviderSettingsSnapshot(settings, providerId);
...
async function getTabSettingsSnapshot(
  tab: TabProviderContext,
  plugin: PidianPlugin,
): Promise<TabProviderSettings> {
  return await ProviderSettingsCoordinator.getProviderSettingsSnapshot(
...
async function updateTabProviderSettings(
  ...
  const snapshot = await getTabSettingsSnapshot(tab, plugin);
  update(snapshot);
  ...
  tab.cachedSettings = snapshot as unknown as Record<string, unknown>;
  
  return snapshot;
}

function refreshTabProviderUI(tab: TabData, plugin: PidianPlugin): void {
 ...
 */
function applyProviderUIGating(tab: TabData, plugin: PidianPlugin): void {
  const capabilities = getTabCapabilities(tab, plugin);
 ...
 */
export async function onProviderAvailabilityChanged(tab: TabData, plugin: PidianPlugin): Promise<void> {
 ...
 */
export async function createTab(options: TabCreateOptions): Promise<TabData> {
  const {
    plugin,
    containerEl,
    conversation,
  ...
  const contentEl = containerEl.createDiv({ cls: 'pidian-tab-content' });
  ...
  const draftModel = isBound ? null : await resolveBlankTabModel(plugin, options.defaultProviderId);
  const initialProviderId = conversation?.providerId ?? DEFAULT_CHAT_PROVIDER_ID;

  const initialSettings = await getTabSettingsSnapshot(
    { conversationId: conversation?.id ?? null, service: null, providerId: initialProviderId, lifecycleState: isBound ? 'bound_cold' : 'blank', draftModel },
    plugin,
  );
  ...
  const tab: TabData = {
    ...
    cachedSettings: initialSettings as unknown as Record<string, unknown>,
    state,
    controllers: {
...
function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  ...
  const viewHeight = textarea.closest('.pidian-container')?.clientHeight ?? window.innerHeight;
...
function buildTabDOM(contentEl: HTMLElement): TabDOMElements {
  const messagesWrapperEl = contentEl.createDiv({ cls: 'pidian-messages-wrapper' });
  const messagesEl = messagesWrapperEl.createDiv({ cls: 'pidian-messages' });
  const welcomeEl = messagesEl.createDiv({ cls: 'pidian-welcome' });
  const statusPanelContainerEl = contentEl.createDiv({ cls: 'pidian-status-panel-container' });
  const inputContainerEl = contentEl.createDiv({ cls: 'pidian-input-container' });
  const queueIndicatorEl = inputContainerEl.createDiv({ cls: 'pidian-input-queue-row' });
  const navRowEl = inputContainerEl.createDiv({ cls: 'pidian-input-nav-row' });
  const inputWrapper = inputContainerEl.createDiv({ cls: 'pidian-input-wrapper' });
  const contextRowEl = inputWrapper.createDiv({ cls: 'pidian-context-row' });
...
function initializeContextManagers(tab: TabData, plugin: PidianPlugin): void {
  const { dom } = tab;
 ...
 */
function initializeInstructionAndTodo(tab: TabData, plugin: PidianPlugin): void {
  const { dom } = tab;
...
function isBangBashEnabled(settings: Record<string, unknown>): boolean {
  return ProviderRegistry.getChatUIConfig(DEFAULT_CHAT_PROVIDER_ID).isBangBashEnabled?.(settings) ?? false;
}
...
function initializeInputToolbar(
  ...
  const inputToolbar = dom.inputWrapper.createDiv({ cls: 'pidian-input-toolbar' });

  // Blank tabs use the PI model list; Pidian is PI-only.
  const blankTabUIConfigProxy = (): ProviderChatUIConfig => {
    const baseConfig = ProviderRegistry.getChatUIConfig(DEFAULT_CHAT_PROVIDER_ID);
    return {
      ...baseConfig,
  ...
  const toolbarComponents = createInputToolbar(inputToolbar, {
    ...
    getSettings: () => tab.cachedSettings as ToolbarSettings,
    getEnvironmentVariables: () => plugin.getActiveEnvironmentVariables(),
    onModelChange: async (model: string) => {
      // For blank tabs, update draft model and derive provider
      if (tab.lifecycleState === 'blank') {
        tab.draftModel = model;
        const newProvider = DEFAULT_CHAT_PROVIDER_ID;
        ...
        if (newProvider === 'pi') {
          try {
            await switchPiDefaultModel(plugin, tab, model);
          } catch (error) {
            console.error('[Tab] Failed to set PI model:', error);
        ...
        await tab.ui.thinkingBudgetSelector?.updateDisplay();
        tab.ui.serviceTierToggle?.updateDisplay();
        await tab.ui.modelSelector?.updateDisplay();
        // Re-render options (provider may have changed reasoning controls)
        await tab.ui.modelSelector?.renderOptions();
        applyProviderUIGating(tab, plugin);
        return;
      }
...
export function initializeTabUI(
  ...
  dom.selectionIndicatorEl = dom.contextRowEl.createDiv({ cls: 'pidian-selection-indicator' });
  dom.selectionIndicatorEl.style.display = 'none';

  dom.browserIndicatorEl = dom.contextRowEl.createDiv({ cls: 'pidian-browser-selection-indicator' });
  dom.browserIndicatorEl.style.display = 'none';

  dom.canvasIndicatorEl = dom.contextRowEl.createDiv({ cls: 'pidian-canvas-indicator' });
 ...
 */
function resolveForkSource(tab: TabData, plugin: PidianPlugin): ForkSource | null {
  ...
  tab.controllers.conversationController = new ConversationController(
    ...
    },
    {
      onNewConversation: async () => {
        ...
        tab.draftModel = await resolveBlankTabModel(plugin, previousProviderId);
 ...
 */
export function wireTabInputEvents(tab: TabData, plugin: PidianPlugin): void {
 ...
 */
export function getTabTitle(tab: TabData, plugin: PidianPlugin): string {
...
export function setupServiceCallbacks(tab: TabData, plugin: PidianPlugin): void {
