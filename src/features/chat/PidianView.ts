...
import { VIEW_TYPE_PIDIAN } from '../../core/types';
...
export class PidianView extends ItemView {
  ...
  constructor(leaf: WorkspaceLeaf, plugin: PidianPlugin) {
    ...
    // overwritten by prototype patching. Hover Editor patches PidianView.prototype.load
  ...
  async refreshModelSelector(): Promise<void> {
    for (const tab of this.tabManager?.getAllTabs() ?? []) {
      onProviderAvailabilityChanged(tab, this.plugin);
      const providerId = getTabProviderId(tab, this.plugin);
      const providerSettings = await ProviderSettingsCoordinator.getProviderSettingsSnapshot(
  ...
  async onOpen() {
    ...
    this.viewContainerEl.addClass('pidian-container');

    const header = this.viewContainerEl.createDiv({ cls: 'pidian-header' });
    this.buildHeader(header);

    this.navRowContent = this.buildNavRowContent();
    this.tabContentEl = this.viewContainerEl.createDiv({ cls: 'pidian-tab-content-container' });
  ...
  private buildHeader(header: HTMLElement) {
    ...
    this.titleSlotEl = header.createDiv({ cls: 'pidian-title-slot' });

    // Logo (hidden when 2+ tabs) — populated by syncHeaderLogo()
    this.logoEl = this.titleSlotEl.createSpan({ cls: 'pidian-logo' });
    this.syncHeaderLogo(DEFAULT_CHAT_PROVIDER_ID);

    // Title text (hidden in header mode when 2+ tabs)
    this.titleTextEl = this.titleSlotEl.createEl('h4', { text: 'Pidian', cls: 'pidian-title-text' });

    // Header actions container (for header mode - initially hidden)
    this.headerActionsEl = header.createDiv({ cls: 'pidian-header-actions pidian-header-actions-slot' });
  ...
  private buildNavRowContent(): HTMLElement {
    ...
    this.tabBarContainerEl.className = 'pidian-tab-bar-container';
    ...
    this.headerActionsContent.className = 'pidian-header-actions';

    // New tab button (plus icon)
    const newTabBtn = this.headerActionsContent.createDiv({ cls: 'pidian-header-btn pidian-new-tab-btn' });
    ...
    const newBtn = this.headerActionsContent.createDiv({ cls: 'pidian-header-btn' });
    ...
    // History dropdown
    const historyContainer = this.headerActionsContent.createDiv({ cls: 'pidian-history-container' });
    const historyBtn = historyContainer.createDiv({ cls: 'pidian-header-btn' });
    setIcon(historyBtn, 'history');
    historyBtn.setAttribute('aria-label', 'Chat history');

    this.historyDropdown = historyContainer.createDiv({ cls: 'pidian-history-menu' });
  ...
  updateLayoutForPosition(): void {
    ...
    this.viewContainerEl.toggleClass('pidian-container--header-mode', isHeaderMode);
  ...
  private wireEventHandlers(): void {
    ...
    // navigating away when Pidian is open as a main-area tab.
