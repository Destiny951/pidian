...
export class SubagentManager {
  ...
  private updateSubagentLabel(
    ...
    if (description) {
      ...
      const labelEl = wrapperEl.querySelector('.pidian-subagent-label') as HTMLElement | null;
      if (labelEl) {
    ...
    if (prompt) {
      info.prompt = prompt;
      const promptEl = wrapperEl.querySelector('.pidian-subagent-prompt-text') as HTMLElement | null;
      if (promptEl) {
