...
export class ImageContextManager {
  ...
  constructor(
    ...
    const fileIndicator = this.previewContainerEl.querySelector('.pidian-file-indicator');
    this.imagePreviewEl = this.previewContainerEl.createDiv({ cls: 'pidian-image-preview' });
  ...
  private setupDragAndDrop() {
    const inputWrapper = this.containerEl.querySelector('.pidian-input-wrapper') as HTMLElement;
    if (!inputWrapper) return;

    this.dropOverlay = inputWrapper.createDiv({ cls: 'pidian-drop-overlay' });
    const dropContent = this.dropOverlay.createDiv({ cls: 'pidian-drop-content' });
  ...
  private handleDragLeave(e: DragEvent) {
    ...
    const inputWrapper = this.containerEl.querySelector('.pidian-input-wrapper');
    if (!inputWrapper) {
  ...
  private renderImagePreview(id: string, image: ImageAttachment) {
    const previewEl = this.imagePreviewEl.createDiv({ cls: 'pidian-image-chip' });

    const thumbEl = previewEl.createDiv({ cls: 'pidian-image-thumb' });
    ...
    const infoEl = previewEl.createDiv({ cls: 'pidian-image-info' });
    const nameEl = infoEl.createSpan({ cls: 'pidian-image-name' });
    nameEl.setText(this.truncateName(image.name, 20));
    nameEl.setAttribute('title', image.name);

    const sizeEl = infoEl.createSpan({ cls: 'pidian-image-size' });
    sizeEl.setText(this.formatSize(image.size));

    const removeEl = previewEl.createSpan({ cls: 'pidian-image-remove' });
    removeEl.setText('\u00D7');
