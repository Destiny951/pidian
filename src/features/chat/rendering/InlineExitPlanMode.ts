...
export class InlineExitPlanMode {
  ...
  render(): void {
    ...
    const titleEl = this.rootEl.createDiv({ cls: 'pidian-plan-inline-title' });
    titleEl.setText('Plan complete');

    this.planContent = this.readPlanContent();
    if (this.planContent) {
      const contentEl = this.rootEl.createDiv({ cls: 'pidian-plan-content-preview' });
      if (this.renderContent) {
        void this.renderContent(contentEl, this.planContent);
      } else {
        contentEl.createDiv({ cls: 'pidian-plan-content-text', text: this.planContent });
      }
    } else if (this.planReadError) {
      this.rootEl.createDiv({
        cls: 'pidian-plan-content-preview pidian-plan-read-error',
    ...
    if (allowedPrompts && Array.isArray(allowedPrompts) && allowedPrompts.length > 0) {
      const permEl = this.rootEl.createDiv({ cls: 'pidian-plan-permissions' });
      permEl.createDiv({ text: 'Requested permissions:', cls: 'pidian-plan-permissions-label' });
      const listEl = permEl.createEl('ul', { cls: 'pidian-plan-permissions-list' });
    ...
    const newSessionRow = actionsEl.createDiv({ cls: 'pidian-ask-item' });
    newSessionRow.addClass('is-focused');
    newSessionRow.createSpan({ text: '\u203A', cls: 'pidian-ask-cursor' });
    newSessionRow.createSpan({ text: '1. ', cls: 'pidian-ask-item-num' });
    newSessionRow.createSpan({ text: 'Approve (new session)', cls: 'pidian-ask-item-label' });
    ...
    const approveRow = actionsEl.createDiv({ cls: 'pidian-ask-item' });
    approveRow.createSpan({ text: '\u00A0', cls: 'pidian-ask-cursor' });
    approveRow.createSpan({ text: '2. ', cls: 'pidian-ask-item-num' });
    approveRow.createSpan({ text: 'Approve (current session)', cls: 'pidian-ask-item-label' });
    ...
    const feedbackRow = actionsEl.createDiv({ cls: 'pidian-ask-item pidian-ask-custom-item' });
    feedbackRow.createSpan({ text: '\u00A0', cls: 'pidian-ask-cursor' });
    feedbackRow.createSpan({ text: '3. ', cls: 'pidian-ask-item-num' });
