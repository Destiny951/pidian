...
export class MessageRenderer {
  ...
  addMessage(msg: ChatMessage): HTMLElement {
    ...
    const msgEl = this.messagesEl.createDiv({
      cls: `pidian-message pidian-message-${msg.role}`,
      attr: {
    ...
    const contentEl = msgEl.createDiv({ cls: 'pidian-message-content', attr: { dir: 'auto' } });
  ...
  updateLiveUserMessage(msg: ChatMessage): void {
    ...
    const toolbar = msgEl.querySelector('.pidian-user-msg-actions') as HTMLElement | null;
    if (toolbar) {
      toolbar.querySelectorAll('.pidian-user-msg-copy-btn').forEach((el) => el.remove());
    }
  ...
  renderMessages(
    ...
    const newWelcomeEl = this.messagesEl.createDiv({ cls: 'pidian-welcome' });
    newWelcomeEl.createDiv({ cls: 'pidian-welcome-greeting', text: getGreeting() });
  ...
  private renderInterruptMessage(): void {
    const msgEl = this.messagesEl.createDiv({ cls: 'pidian-message pidian-message-assistant' });
    const contentEl = msgEl.createDiv({ cls: 'pidian-message-content', attr: { dir: 'auto' } });
    this.appendInterruptIndicator(contentEl);
  }
  ...
  private appendInterruptIndicator(contentEl: HTMLElement): void {
    const textEl = contentEl.createDiv({ cls: 'pidian-text-block' });
    textEl.innerHTML = '<span class="pidian-interrupted">Interrupted</span> <span class="pidian-interrupted-hint">· What should Pidian do instead?</span>';
  }
  ...
  private renderAssistantContent(msg: ChatMessage, contentEl: HTMLElement): void {
    ...
    if (msg.durationSeconds && msg.durationSeconds > 0 && !hasCompactBoundary) {
      ...
      const footerEl = contentEl.createDiv({ cls: 'pidian-response-footer' });
      footerEl.createSpan({
        text: `* ${flavorWord} for ${formatDurationMmSs(msg.durationSeconds)}`,
        cls: 'pidian-baked-duration',
      });
    }
  }
  ...
  renderMessageImages(containerEl: HTMLElement, images: ImageAttachment[]): void {
    const imagesEl = containerEl.createDiv({ cls: 'pidian-message-images' });

    for (const image of images) {
      const imageWrapper = imagesEl.createDiv({ cls: 'pidian-message-image' });
  ...
  showFullImage(image: ImageAttachment): void {
    ...
    const overlay = document.body.createDiv({ cls: 'pidian-image-modal-overlay' });
    const modal = overlay.createDiv({ cls: 'pidian-image-modal' });
    ...
    const closeBtn = modal.createDiv({ cls: 'pidian-image-modal-close' });
    closeBtn.setText('\u00D7');
  ...
  async renderContent(el: HTMLElement, markdown: string): Promise<void> {
    ...
    try {
      ...
      el.querySelectorAll('pre').forEach((pre) => {
        ...
        if (pre.parentElement?.classList.contains('pidian-code-wrapper')) return;

        // Create wrapper
        const wrapper = createEl('div', { cls: 'pidian-code-wrapper' });
  ...
  addTextCopyButton(textEl: HTMLElement, markdown: string): void {
    const copyBtn = textEl.createSpan({ cls: 'pidian-text-copy-btn' });
  ...
  refreshActionButtons(msg: ChatMessage, allMessages?: ChatMessage[], index?: number): void {
    ...
    if (this.rewindCallback && !msgEl.querySelector('.pidian-message-rewind-btn')) {
      this.addRewindButton(msgEl, msg.id);
    }
    if (this.forkCallback && !msgEl.querySelector('.pidian-message-fork-btn')) {
      this.addForkButton(msgEl, msg.id);
    }
  ...
  private cleanupLiveMessageEl(msgId: string, msgEl: HTMLElement): void {
    const needsRewind = this.rewindCallback && !msgEl.querySelector('.pidian-message-rewind-btn');
    const needsFork = this.forkCallback && !msgEl.querySelector('.pidian-message-fork-btn');
  ...
  private getOrCreateActionsToolbar(msgEl: HTMLElement): HTMLElement {
    const existing = msgEl.querySelector('.pidian-user-msg-actions') as HTMLElement | null;
    if (existing) return existing;
    return msgEl.createDiv({ cls: 'pidian-user-msg-actions' });
  }

  private addUserCopyButton(msgEl: HTMLElement, content: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const copyBtn = toolbar.createSpan({ cls: 'pidian-user-msg-copy-btn' });
    copyBtn.innerHTML = MessageRenderer.COPY_ICON;
  ...
  private addRewindButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'pidian-message-rewind-btn' });
  ...
  private addForkButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'pidian-message-fork-btn' });
    if (toolbar.firstChild !== btn) toolbar.insertBefore(btn, toolbar.firstChild);
  ...
  private renderSkillBlock(contentEl: HTMLElement, skillName: string, args?: string): void {
    const skillEl = contentEl.createDiv({ cls: 'pidian-skill-call' });

    const header = skillEl.createDiv({ cls: 'pidian-tool-header' });
    ...
    if (args) {
      const argsEl = skillEl.createDiv({ cls: 'pidian-skill-args' });
      argsEl.setText(args);
    }
  }
  ...
  private renderUserContextBlocks(contentEl: HTMLElement, blocks?: ChatMessage['contentBlocks']): void {
    ...
    const detailsEl = contentEl.createEl('details', { cls: 'pidian-user-context' });
    const summaryEl = detailsEl.createEl('summary', { cls: 'pidian-user-context-summary' });
    summaryEl.setText(`Context (${contextBlocks.length})`);

    const bodyEl = detailsEl.createDiv({ cls: 'pidian-user-context-list' });
    for (const block of contextBlocks) {
      const parsed = this.parseContextTag(block.tag, block.content);
      const cardEl = bodyEl.createDiv({ cls: 'pidian-user-context-card' });

      const headerEl = cardEl.createDiv({ cls: 'pidian-user-context-header' });
      const iconEl = headerEl.createSpan({ cls: 'pidian-user-context-icon' });
      setIcon(iconEl, parsed.icon);

      const metaEl = headerEl.createDiv({ cls: 'pidian-user-context-meta' });
      metaEl.createDiv({ cls: 'pidian-user-context-title', text: parsed.title });
      if (parsed.summary) {
        metaEl.createDiv({ cls: 'pidian-user-context-subtitle', text: parsed.summary });
      }

      if (parsed.details) {
        const detailsNode = cardEl.createEl('details', { cls: 'pidian-user-context-details' });
        detailsNode.createEl('summary', { text: '查看内容' });
        detailsNode.createEl('pre', {
          cls: 'pidian-user-context-content',
          text: parsed.details,
        });
      }
    }
