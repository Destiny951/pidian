...
export function renderTodoItems(
  ...
  for (const todo of todos) {
    const item = container.createDiv({ cls: `pidian-todo-item pidian-todo-${todo.status}` });

    const icon = item.createSpan({ cls: 'pidian-todo-status-icon' });
    icon.setAttribute('aria-hidden', 'true');
    setIcon(icon, getTodoStatusIcon(todo.status));

    const text = item.createSpan({ cls: 'pidian-todo-text' });
