...
function appendToolLink(parent: HTMLElement, title: string, url: string): void {
  const linkEl = parent.createEl('a', { cls: 'pidian-tool-link' });
  linkEl.setAttribute('href', url);
  linkEl.setAttribute('target', '_blank');
  linkEl.setAttribute('rel', 'noopener noreferrer');

  const iconEl = linkEl.createSpan({ cls: 'pidian-tool-link-icon' });
  setIcon(iconEl, 'external-link');

  linkEl.createSpan({ cls: 'pidian-tool-link-title', text: title });
}
...
function renderWebSearchActionExpanded(container: HTMLElement, input: Record<string, unknown>): boolean {
  ...
  const linesEl = container.createDiv({ cls: 'pidian-tool-lines' });

  switch (data.actionType) {
    case 'open_page':
      linesEl.createDiv({ cls: 'pidian-tool-line', text: 'Open page' });
      if (data.url) {
        appendToolLink(linesEl, data.url, data.url);
      } else {
        linesEl.createDiv({ cls: 'pidian-tool-line', text: 'URL unavailable' });
      }
      return true;

    case 'find_in_page':
      linesEl.createDiv({ cls: 'pidian-tool-line', text: 'Find in page' });
      if (data.url) {
        appendToolLink(linesEl, data.url, data.url);
      } else {
        linesEl.createDiv({ cls: 'pidian-tool-line', text: 'URL unavailable' });
      ...
      if (data.pattern) {
        linesEl.createDiv({ cls: 'pidian-tool-line', text: `Pattern: ${data.pattern}` });
      }
      return true;
    ...
    default: {
      ...
      for (const query of alternateQueries.slice(0, 4)) {
        linesEl.createDiv({ cls: 'pidian-tool-line', text: `Alt query: ${query}` });
      }
      if (alternateQueries.length > 4) {
        linesEl.createDiv({
          cls: 'pidian-tool-truncated',
...
function renderWebSearchExpanded(
  ...
  if (parsed && parsed.links.length > 0) {
    const linksEl = container.createDiv({ cls: 'pidian-tool-lines' });
    for (const link of parsed.links) {
      appendToolLink(linksEl, link.title, link.url);
    }

    if (parsed.summary) {
      const summaryEl = container.createDiv({ cls: 'pidian-tool-web-summary' });
  ...
  container.createDiv({ cls: 'pidian-tool-empty', text: 'No result' });
}

function renderFileSearchExpanded(container: HTMLElement, result: string): void {
  const lines = result.split(/\r?\n/).filter(line => line.trim());
  if (lines.length === 0) {
    container.createDiv({ cls: 'pidian-tool-empty', text: 'No matches found' });
    return;
  }
...
function renderLinesExpanded(
  ...
  for (const line of displayLines) {
    ...
    const lineEl = linesEl.createDiv({ cls: 'pidian-tool-line' });
...
function renderToolSearchExpanded(container: HTMLElement, result: string): void {
  ...
  for (const name of toolNames) {
    const lineEl = container.createDiv({ cls: 'pidian-tool-search-item' });
    const iconEl = lineEl.createSpan({ cls: 'pidian-tool-search-icon' });
...
function renderApplyPatchExpanded(
  ...
  if (parsedDiffs.length > 0) {
    for (const fileDiff of parsedDiffs) {
      const sectionEl = container.createDiv({ cls: 'pidian-tool-patch-section' });
      ...
      const pathText = fileDiff.movedTo
        ...
        : fileDiff.filePath;
      sectionEl.createDiv({
        cls: 'pidian-tool-patch-header',
        text: `${fileDiff.operation}: ${pathText}${statsSuffix}`,
      });

      if (fileDiff.operation === 'delete' && fileDiff.diffLines.length === 0) {
        sectionEl.createDiv({ cls: 'pidian-tool-empty', text: 'File deleted' });
        continue;
      }

      if (fileDiff.diffLines.length === 0) {
        sectionEl.createDiv({ cls: 'pidian-tool-empty', text: 'No textual diff available' });
        continue;
      }

      const diffRow = sectionEl.createDiv({ cls: 'pidian-write-edit-diff-row' });
      const diffEl = diffRow.createDiv({ cls: 'pidian-write-edit-diff' });
  ...
  if (changes.length > 0) {
    ...
    for (const change of changes) {
      ...
      if (!path) continue;
      linesEl.createDiv({ cls: 'pidian-tool-line', text: `${kind}: ${path}` });
    }
    return;
  }
...
function resetStatusElement(statusEl: HTMLElement, statusClass: string, ariaLabel: string): void {
  statusEl.className = 'pidian-tool-status';
  statusEl.empty();
...
export function renderTodoWriteResult(
  ...
  container.empty();
  container.addClass('pidian-todo-panel-content');
  container.addClass('pidian-todo-list-container');

  const todos = input.todos as TodoItem[] | undefined;
  if (!todos || !Array.isArray(todos)) {
    const item = container.createSpan({ cls: 'pidian-tool-result-item' });
...
function createToolElementStructure(
  ...
  const toolEl = parentEl.createDiv({ cls: 'pidian-tool-call' });

  const header = toolEl.createDiv({ cls: 'pidian-tool-header' });
  header.setAttribute('tabindex', '0');
  header.setAttribute('role', 'button');

  const iconEl = header.createSpan({ cls: 'pidian-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = header.createSpan({ cls: 'pidian-tool-name' });
  nameEl.setText(getToolName(toolCall.name, toolCall.input));

  const summaryEl = header.createSpan({ cls: 'pidian-tool-summary' });
  ...
  const statusEl = header.createSpan({ cls: 'pidian-tool-status' });

  const content = toolEl.createDiv({ cls: 'pidian-tool-content' });
...
function renderAskUserQuestionResult(container: HTMLElement, toolCall: ToolCallInfo): boolean {
  ...
  const reviewEl = container.createDiv({ cls: 'pidian-ask-review' });
  for (let i = 0; i < questions.length; i++) {
    const q = questions[i];
    const answer = formatAnswer(
      (q.id ? answers[q.id] : undefined) ?? answers[q.question]
    );
    const pairEl = reviewEl.createDiv({ cls: 'pidian-ask-review-pair' });
    pairEl.createDiv({ text: `${i + 1}.`, cls: 'pidian-ask-review-num' });
    const bodyEl = pairEl.createDiv({ cls: 'pidian-ask-review-body' });
    bodyEl.createDiv({ text: q.question, cls: 'pidian-ask-review-q-text' });
    bodyEl.createDiv({
      text: answer || 'Not answered',
      cls: answer ? 'pidian-ask-review-a-text' : 'pidian-ask-review-empty',
    });
  }
...
function renderAskUserQuestionFallback(container: HTMLElement, toolCall: ToolCallInfo, initialText?: string): void {
  ...
  for (let questionIndex = 0; questionIndex < questions.length; questionIndex++) {
    ...
    const reviewEl = container.createDiv({ cls: 'pidian-ask-review' });
    const pairEl = reviewEl.createDiv({ cls: 'pidian-ask-review-pair' });
    pairEl.createDiv({ text: `${questionIndex + 1}.`, cls: 'pidian-ask-review-num' });
    const bodyEl = pairEl.createDiv({ cls: 'pidian-ask-review-body' });
    bodyEl.createDiv({ text: question.question, cls: 'pidian-ask-review-q-text' });

    if (!Array.isArray(question.options) || question.options.length === 0) {
      bodyEl.createDiv({ cls: 'pidian-ask-review-empty', text: 'No options recorded' });
      continue;
    }

    const listEl = bodyEl.createDiv({ cls: 'pidian-ask-list' });
...
function renderAskUserQuestionOption(
  ...
  const itemEl = parentEl.createDiv({ cls: 'pidian-ask-item is-disabled' });

  if (isMultiSelect) {
    itemEl.createDiv({ cls: 'pidian-ask-check', text: '[ ] ' });
  } else {
    itemEl.createDiv({ cls: 'pidian-ask-item-num', text: `${optionIndex + 1}. ` });
  }

  const contentEl = itemEl.createDiv({ cls: 'pidian-ask-item-content' });
  const labelRowEl = contentEl.createDiv({ cls: 'pidian-ask-label-row' });
  labelRowEl.createDiv({ cls: 'pidian-ask-item-label', text: option.label });

  if (option.description) {
    contentEl.createDiv({ cls: 'pidian-ask-item-desc', text: option.description });
  }
}

function contentFallback(container: HTMLElement, text: string): void {
  const resultRow = container.createDiv({ cls: 'pidian-tool-result-row' });
  const resultText = resultRow.createSpan({ cls: 'pidian-tool-result-text' });
...
function createCurrentTaskPreview(
  ...
  const currentTaskEl = header.createSpan({ cls: 'pidian-tool-current' });
...
export function updateToolCallResult(
  ...
  if (toolCall.name === TOOL_TODO_WRITE) {
    const statusEl = toolEl.querySelector('.pidian-tool-status') as HTMLElement;
    if (statusEl) {
      setTodoWriteStatus(statusEl, toolCall.input);
    }
    const content = toolEl.querySelector('.pidian-tool-content') as HTMLElement;
    if (content) {
      renderTodoWriteResult(content, toolCall.input);
    }
    const nameEl = toolEl.querySelector('.pidian-tool-name') as HTMLElement;
    if (nameEl) {
      nameEl.setText(getToolName(toolCall.name, toolCall.input));
    }
    const currentTaskEl = toolEl.querySelector('.pidian-tool-current') as HTMLElement;
    if (currentTaskEl) {
