...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'pidian-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'pidian-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'pidian-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'pidian-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.pidian-thinking-header');
  if (header) {
