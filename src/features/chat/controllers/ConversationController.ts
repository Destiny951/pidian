...
export class ConversationController {
  ...
  async createNew(options: { force?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      welcomeEl.createDiv({ cls: 'pidian-welcome-greeting', text: this.getGreeting() });
  ...
  private renderHistoryItems(
    ...
    const dropdownHeader = container.createDiv({ cls: 'pidian-history-header' });
    dropdownHeader.createSpan({ text: 'Conversations' });

    const list = container.createDiv({ cls: 'pidian-history-list' });
    const allConversations = plugin.getConversationList();

    if (allConversations.length === 0) {
      list.createDiv({ cls: 'pidian-history-empty', text: 'No conversations' });
      return;
    }
    ...
    for (const conv of conversations) {
      ...
      const item = list.createDiv({
        cls: `pidian-history-item${isCurrent ? ' active' : ''}`,
      });

      const iconEl = item.createDiv({ cls: 'pidian-history-item-icon' });
      setIcon(iconEl, isCurrent ? 'message-square-dot' : 'message-square');

      const content = item.createDiv({ cls: 'pidian-history-item-content' });
      const titleEl = content.createDiv({ cls: 'pidian-history-item-title', text: conv.title });
      titleEl.setAttribute('title', conv.title);
      content.createDiv({
        cls: 'pidian-history-item-date',
      ...
      const actions = item.createDiv({ cls: 'pidian-history-item-actions' });

      // Show regenerate button if title generation failed, or loading indicator if pending
      if (conv.titleGenerationStatus === 'pending') {
        const loadingEl = actions.createEl('span', { cls: 'pidian-action-btn pidian-action-loading' });
        setIcon(loadingEl, 'loader-2');
        loadingEl.setAttribute('aria-label', 'Generating title...');
      } else if (conv.titleGenerationStatus === 'failed') {
        const regenerateBtn = actions.createEl('button', { cls: 'pidian-action-btn' });
      ...
      const renameBtn = actions.createEl('button', { cls: 'pidian-action-btn' });
      ...
      const deleteBtn = actions.createEl('button', { cls: 'pidian-action-btn pidian-delete-btn' });
  ...
  private showRenameInput(item: HTMLElement, convId: string, currentTitle: string): void {
    const titleEl = item.querySelector('.pidian-history-item-title') as HTMLElement;
    if (!titleEl) return;

    const input = document.createElement('input');
    input.type = 'text';
    input.className = 'pidian-rename-input';
  ...
  getGreeting(): string {
    ...
    const getTimeGreetings = (): string[] => {
      if (hour >= 5 && hour < 12) {
        return [personalize('Good morning'), 'Coffee and Pidian time?'];
  ...
  initializeWelcome(): void {
    ...
    if (!welcomeEl.querySelector('.pidian-welcome-greeting')) {
      welcomeEl.createDiv({ cls: 'pidian-welcome-greeting', text: this.getGreeting() });
    }
  ...
  // ============================================
  // History Dropdown Rendering (for PidianView)
  // ============================================

  /**
   * Renders the history dropdown content to a provided container.
   * Used by PidianView to render the dropdown with custom selection callback.
   */
