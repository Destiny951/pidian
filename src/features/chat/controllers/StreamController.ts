...
export class StreamController {
    ...
    switch (chunk.type) {
      ...
      case 'usage': {
        ...
        if (!state.ignoreUsageUpdates) {
          const activeModel = await this.getActiveProviderModel();
  ...
  private handleRegularToolUse(
    ...
    if (existingToolCall) {
      ...
      if (Object.keys(newInput).length > 0) {
        ...
        if (toolEl) {
          const nameEl = toolEl.querySelector('.pidian-tool-name') as HTMLElement | null
            ?? toolEl.querySelector('.pidian-write-edit-name') as HTMLElement | null;
          if (nameEl) {
            nameEl.setText(getToolName(existingToolCall.name, existingToolCall.input));
          }
          const summaryEl = toolEl.querySelector('.pidian-tool-summary') as HTMLElement | null
            ?? toolEl.querySelector('.pidian-write-edit-summary') as HTMLElement | null;
          if (summaryEl) {
  ...
  private async getActiveProviderModel(): Promise<string | undefined> {
    const providerId = this.deps.getAgentService?.()?.providerId;
    if (!providerId) {
      return undefined;
    }

    const settings = await ProviderSettingsCoordinator.getProviderSettingsSnapshot(
  ...
  async appendText(text: string): Promise<void> {
    ...
    if (!state.currentTextEl) {
      state.currentTextEl = state.currentContentEl.createDiv({ cls: 'pidian-text-block' });
  ...
  showRunStatus(text = 'Agent is still working...'): void {
    ...
    if (state.streamStatusEl) {
      const textEl = state.streamStatusEl.querySelector('.pidian-stream-status-text') as HTMLElement | null;
      if (textEl) {
        textEl.setText(text);
      }
      this.keepRunStatusAtBottom();
    ...
    const statusEl = state.currentContentEl.createDiv({ cls: 'pidian-stream-status' });
    statusEl.createSpan({ cls: 'pidian-stream-status-spinner', attr: { 'aria-hidden': 'true' } });
    statusEl.createSpan({ cls: 'pidian-stream-status-text', text });
    const timerSpan = statusEl.createSpan({ cls: 'pidian-stream-status-hint' });
    const updateTimer = () => {
  ...
  private renderCompactBoundary(): void {
    const { state } = this.deps;
    if (!state.currentContentEl) return;
    const el = state.currentContentEl.createDiv({ cls: 'pidian-compact-boundary' });
    el.createSpan({ cls: 'pidian-compact-boundary-label', text: 'Conversation compacted' });
  }
