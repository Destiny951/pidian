...
import { DiffApprovalView } from '../../diff-view/DiffApprovalView';
...
interface FileMutationPreviewPayload {
  provider: 'pi';
...
function getFileMutationPreviewPayload(value: unknown): FileMutationPreviewPayload | null {
  if (!value || typeof value !== 'object') return null;
  const payload = value as Partial<FileMutationPreviewPayload>;
  if (payload.provider !== 'pi' || payload.kind !== 'file-mutation-preview') return null;
  const request = payload.request;
  const preview = request?.preview;
  if (!request || !preview || !Array.isArray(preview.diffLines)) return null;
  if (request.toolName !== 'edit' && request.toolName !== 'write') return null;
  return payload as FileMutationPreviewPayload;
}
...
export class InputController {
    ...
    try {
      ...
      if (!wasInvalidated && state.streamGeneration === streamGeneration) {
        ...
        if (didCancelThisTurn && !state.pendingNewSessionPlan) {
          await streamController.appendText('\n\n<span class="pidian-interrupted">Interrupted</span> <span class="pidian-interrupted-hint">· What should Pidian do instead?</span>');
        }
        streamController.hideRunStatus?.();
        state.isStreaming = false;
        state.cancelRequested = false;
        ...
        if (!didCancelThisTurn && !hasCompactBoundary) {
          ...
          if (durationSeconds > 0) {
            ...
            if (state.currentContentEl) {
              const footerEl = state.currentContentEl.createDiv({ cls: 'pidian-response-footer' });
              footerEl.createSpan({
  ...
  updateQueueIndicator(): void {
    ...
    if (visibleQueuedMessage) {
      ...
      indicatorEl.createSpan({
        cls: 'pidian-queue-indicator-text',
      ...
      if (state.queuedMessage && this.canSteerQueuedMessage()) {
        const steerButton = indicatorEl.createEl('button', {
          cls: 'pidian-queue-indicator-action',
  ...
  async handleApprovalRequest(
    ...
    const previewPayload = getFileMutationPreviewPayload(approvalOptions?.additionalPermissions);

    if (previewPayload && !previewPayload.request.preview.error) {
      const { request } = previewPayload;
      const { preview } = request;

      const result = await DiffApprovalView.showDiff(
        this.deps.plugin,
        {
      ...
      if (result.decision === 'approve') {
        if (result.editedContent !== undefined) {
          preview.proposedContent = result.editedContent;
        }
        return 'allow';
      }
    ...
    const headerEl = parentEl.createDiv({ cls: 'pidian-ask-approval-info' });
    headerEl.remove();

    const toolEl = headerEl.createDiv({ cls: 'pidian-ask-approval-tool' });
    const iconEl = toolEl.createSpan({ cls: 'pidian-ask-approval-icon' });
    iconEl.setAttribute('aria-hidden', 'true');
    setToolIcon(iconEl, toolName);
    toolEl.createSpan({ text: toolName, cls: 'pidian-ask-approval-tool-name' });

    if (approvalOptions?.decisionReason) {
      headerEl.createDiv({ text: approvalOptions.decisionReason, cls: 'pidian-ask-approval-reason' });
    }
    if (approvalOptions?.blockedPath) {
      headerEl.createDiv({ text: approvalOptions.blockedPath, cls: 'pidian-ask-approval-blocked-path' });
    }
    if (approvalOptions?.agentID) {
      headerEl.createDiv({ text: `Agent: ${approvalOptions.agentID}`, cls: 'pidian-ask-approval-agent' });
    }

    headerEl.createDiv({ text: description, cls: 'pidian-ask-approval-desc' });

    if (previewPayload?.request.preview.error) {
      headerEl.createDiv({ text: previewPayload.request.preview.error, cls: 'pidian-ask-approval-preview-error' });
    }
