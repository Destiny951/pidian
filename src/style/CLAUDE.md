...
├── settings/       # base (shared .pidian-sp-* panel layout), env-snippets, slash-settings, mcp-settings, plugin-settings, agent-settings
├── accessibility.css
...
- **Prefix**: Pidian-owned classes should use the `.pidian-` prefix; shared Obsidian host selectors and generic state classes may remain unprefixed
- **BEM-lite**: Prefer `.pidian-{block}`, `.pidian-{block}-{element}`, `.pidian-{block}--{modifier}` for Pidian-owned selectors
